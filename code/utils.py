...
# 'create_examples_with_text'

# Extends 'create_examples_from_batch' to include the original string used to generate the
# example, along with a numerical value indicating the character offset -- the number of
# non-whitespace characters removed from the beginning of the example due to random truncation.

def create_examples_with_text(batch, tokenizer, block_size, short_seq_probability, nsp_probability):
            ...
            if i == len(document) - 1 or current_length >= target_seq_length:
                if current_chunk:
                    ...
                    if len(current_chunk) == 1 or random.random() < nsp_probability:
                        ...
                        random_document = documents[random_document_index]
                        random_text = texts[random_document_index]
                        random_start = random.randint(0, len(random_document) - 1)
                        for j in range(random_start, len(random_document)):
                            tokens_b.extend(random_document[j])
                            text_b.append(random_text[j])
                            if len(tokens_b) >= target_b_length:
                                break
                        # We didn't actually use these segments so we "put them back" so
                        # they don't go to waste.
                    ...
                    def truncate_seq_pair(tokens_a, tokens_b, max_num_tokens):
                        ...
                        while True:
                            ...
                            # back to add more randomness and avoid biases.
                            if random.random() < 0.5:
                                trunc_tok = tokenizer.convert_ids_to_tokens(trunc_tokens[0])
                                trunc_len = len(trunc_tok) if not trunc_tok.startswith('##') else len(trunc_tok) - 2
                                trunc_list[trunc_index] += trunc_len
                                del trunc_tokens[0]
                            else:
                                trunc_tokens.pop()
                        return trunc_list
                    ascii_a = [ord(c) for c in " ".join(text_a) + " " ]
                    ascii_b = [ord(c) for c in " ".join(text_b) ]
                    ascii_joint = ascii_a + ascii_b
            
                    # Call to function copied from LineByLineWithSOPTextDataset class -tr
                    trunc_list = truncate_seq_pair(tokens_a, tokens_b, max_num_tokens)
                    if not (len(tokens_a) >= 1):
                        raise ValueError(f"Length of sequence a is {len(tokens_a)} which must be no less than 1")
                    if not (len(tokens_b) >= 1):
                        raise ValueError(f"Length of sequence b is {len(tokens_b)} which must be no less than 1")
                    ...
                    example = {
                        ...
                        "token_type_ids": torch.tensor(token_type_ids, dtype=torch.long),
                        "next_sentence_label": torch.tensor(1 if is_random_next else 0, dtype=torch.long),
                        "ascii": torch.tensor(ascii_joint, dtype=torch.long),
                        "ascii_type_idx": torch.tensor(len(ascii_a), dtype=torch.long),
                        "trunc_chars": torch.tensor(trunc_list, dtype=torch.long),
                    }
    ...
    # Examples must be formatted as a dictionary for processing by model -tr
    ex_keys = ["input_ids", "token_type_ids", "next_sentence_label", "ascii", "ascii_type_idx", "trunc_chars"]
    return { k:[examples[i][k] for i in range(len(examples))] for k in ex_keys}
...
def deprocess_example(example,tokenizer,pass_text=False):
    ...
    if pass_text:
        ascii_idx = example['ascii_type_idx'][0].item()
        
        example_dict['sent_a']['text'] = "".join([chr(c) for c in example['ascii'][0][:ascii_idx]])
        example_dict['sent_a']['trunc'] = example['trunc_chars'][0][0].item()

        example_dict['sent_b']['text'] = "".join([chr(c) for c in example['ascii'][0][ascii_idx:]])
        example_dict['sent_b']['trunc'] = example['trunc_chars'][0][1].item()
