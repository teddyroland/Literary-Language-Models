...
from utils import create_examples_from_batch, create_one_sentence_examples, DataCollatorForWholeWordMaskAndNSP, deprocess_example
...
arg_parser.add_argument('--single_sent', action='store_true')
arg_parser.add_argument('--pass_text', action='store_true')

args = arg_parser.parse_args()

def create_dataloader(data_dir, tokenizer, single_sent = False):
    ...
    if single_sent:
        example_fn = create_one_sentence_examples
...
test_dataloader = create_dataloader(args.data_dir, bert_tokenizer, args.single_sent)

ex_list = []
for step, inputs in enumerate(test_dataloader):
    if step % 10 == 0:
        input_dict = deprocess_example(inputs, bert_tokenizer, args.pass_text)
